          ...
          <SimpleBar style={{ maxHeight: 300 }}>
            <Col>
              <Row className="flex-column justify-content-start">
                {selectedItem.chooseSize ? (
                  <>
                    ...
                    <ul className={"itemList"}>
                      {Object.keys(selectedItem.chooseSize).map((key) => {
                        let cost = parseFloat(
                          selectedItem.chooseSize[key]
                        ).toFixed(2);
                        if (
                          selectedItem.itemName === "Agua Fresca" ||
                          selectedItem.itemName === "Mangonada"
                        ) {
                          return (
                            <li>
                              <label htmlFor={key}>
                        ...
                        } else if (selectedItem.itemName === "Ice Cream") {
                          return (
                            <li>
                              <label htmlFor={key}>
                                {key} Scoop: ${cost}
                  ...
                  <div className="itemList">
                    <div>
                      <label>
                        Bag: ${parseFloat(selectedItem.price).toFixed(2)}
                        <input
                          type={"radio"}
                ...
                {errors[selectedItem.itemName]?.type === "required" && (
                  <p className={"d-flex justify-content-start"}>
              ...
              <Row className="flex-column justify-content-start">
                {selectedItem.typeOfChips && (
                  <>
                    ...
                    <ul className={"itemList"}>
                      {selectedItem.typeOfChips.map((chips) => (
                        <li>
                          <label htmlFor={chips}>
                            {chips}
                            <input
                  ...
                  <>
                    ...
                    <ul className={"itemList"}>
                      {selectedItem.flavor.map((flavor, index) => {
                        if (selectedItem.itemName === "Ice Cream") {
                          return (
                            <li>
                              <label htmlFor={flavor}>
                                ...
                                <input
                                  ...
                                  ref={register({
                                    ...
                                    validate: () => {
                                      return (
                                        flavors.length <= parseInt(numOfScoops)
                                      );
                                    },
                                  })}
                                />
